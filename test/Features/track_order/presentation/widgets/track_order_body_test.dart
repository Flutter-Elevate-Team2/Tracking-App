...
@GenerateNiceMocks([
  MockSpec<OrderStatusViewModel>(),
  MockSpec<CompleteOrderViewModel>(),
])
import 'track_order_body_test.mocks.dart';

void main() {
  late MockOrderStatusViewModel mockViewModel;
  late MockCompleteOrderViewModel mockCompleteOrderViewModel;
  ...
  setUp(() {
    ...
    mockCompleteOrderViewModel = MockCompleteOrderViewModel();

    // Default stub for CompleteOrderViewModel
    when(
      mockCompleteOrderViewModel.state,
    ).thenReturn(const CompleteOrderState());
    when(
      mockCompleteOrderViewModel.stream,
  ...
  Widget createWidgetUnderTest() {
    return MaterialApp(
      ...
      home: MultiBlocProvider(
        providers: [
          BlocProvider<OrderStatusViewModel>.value(value: mockViewModel),
          BlocProvider<CompleteOrderViewModel>.value(
            value: mockCompleteOrderViewModel,
          ),
        ],
  ...
  group('TrackOrderBody Widget Tests', () {
    testWidgets('should render SizedBox.shrink when order data is null', (
      tester,
    ) async {
      when(mockViewModel.state).thenReturn(
        const TrackOrderStatusState(orderState: BaseState(data: null)),
      );
    ...
    testWidgets(
      'should render full page content when data is loaded successfully',
      (tester) async {
        when(mockViewModel.state).thenReturn(
          TrackOrderStatusState(orderState: BaseState(data: fakeOrder)),
        );
        when(mockViewModel.stream).thenAnswer((_) => const Stream.empty());
    ...
    testWidgets(
      'should show SnackBar when back button is pressed and order is not delivered',
      (tester) async {
        when(mockViewModel.state).thenReturn(
          TrackOrderStatusState(orderState: BaseState(data: fakeOrder)),
        );
    ...
    testWidgets(
      'should reflect loading state in stepper when status is updating',
      (tester) async {
        // status "accepted" → index 0, isLoading: true → LinearProgressIndicator at index 1
        when(mockViewModel.state).thenReturn(
