...
void main() {
  ...
  setUp(() async {
    ...
    when(mockApplyViewModel.state).thenReturn(ApplyStates(applyState: BaseState(isLoading: false)));
    when(mockApplyViewModel.stream).thenAnswer((_) => Stream.value(ApplyStates(applyState: BaseState(isLoading: false))));
    when(mockVehicleViewModel.state).thenReturn(VehicleStates(vehiclesState: BaseState(data: [])));
    when(mockVehicleViewModel.stream).thenAnswer((_) => Stream.value(VehicleStates(vehiclesState: BaseState(data: []))));
  ...
  group('ApplyForm test', () {
    ...
    testWidgets('Should navigate to success when data is received', (tester) async {
      ...
      when(mockApplyViewModel.stream).thenAnswer((_) => Stream.value(successState));
    ...
    testWidgets('Should cover error SnackBar in listener', (tester) async {
      final errorState = ApplyStates(
        applyState: BaseState(errorMessage: "Validation Error", isLoading: false),
      );
    ...
    testWidgets('Should cover isLoading block in listener', (tester) async {
      final loadingState = ApplyStates(
        applyState: BaseState(isLoading: true),
      );

      when(mockApplyViewModel.state).thenReturn(loadingState);
      when(mockApplyViewModel.stream).thenAnswer((_) => Stream.value(loadingState));
    ...
    testWidgets('Should trigger form validation on button press', (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
