...
void main() {
  ...
  Widget createWidgetUnderTest({Function(VehicleEntity?)? onChanged}) {
    return MaterialApp(
      ...
      home: Scaffold(
        body: VehicleTypeField(onChanged: onChanged),
      ),
    );
  }
  ...
  group('VehicleTypeField Tests', () {
    ...
    testWidgets('Should call onChanged and update ViewModel when a vehicle is selected', (tester) async {
    ...
    testWidgets('Should display error message when vehiclesState has an error', (tester) async {
    ...
    testWidgets('Should initialize with GetAllVehiclesEvent on create', (tester) async {
      final state = VehicleStates(
      ...
      verify(mockViewModel.doIntent(argThat(isA<GetAllVehiclesEvent>()))).called(1);
    });
  });
}